...
func (c *playServiceProtobufClient) Play(ctx context.Context, in *UserTrack) (<-chan TrackDataOrError, error) {
	...
	respStream := make(chan TrackDataOrError)
	go func() {
		defer func() {
			resp.Body.Close()
			close(respStream)
		...
		for {
			if err = reader.Read(out); err != nil {
				...
				respStream <- TrackDataOrError{Err: err}
				return
			}
			respStream <- TrackDataOrError{Msg: out}
		}
	}()
...
func (s *playServiceServer) servePlayProtobuf(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	...
	// Call service method
	var respContent <-chan TrackDataOrError
